...
const PostHeader = observer((data,v) => {
    ...
    data = data.data
    function setProdParameters() {
        if (document.getElementById('main_slider_wrapper' + data.id) || document.getElementById('slider_wrapper' + data.id)) {
            let obj1 = JSON.parse(data.obj || '{}');
            obj1.scroll = "Наведением мыши";
            obj1.sliderButtons = "Точки под слайдером";
            obj1.sliderColor = "#7d7d7d";
            obj1.isProduct = true;
            obj1.autoScroll = "false";
            obj1.trueHeight = "false";
            obj1.fullScreen = "false";
            data.obj = JSON.stringify(obj1);
        }
    }
    setProdParameters();
    useEffect(()=>{
        setProdParameters();
    },[document.getElementById('slider_wrapper' + data.id)])
    ...
    useEffect(()=>{
        if(document.getElementById('block'+data.id) && (h1 !== document.getElementById('block'+data.id).getBoundingClientRect().height || w1 !== document.getElementById('block'+data.id).getBoundingClientRect().width)){
            if(document.getElementById('sl'+data.id+'CoI0')){
                document.getElementById('sl'+data.id+'CoI0').style.transitionDuration = '0s'
            }
            if (JSON.parse(data.obj || '{}').isProduct) {
                if (document.getElementById('main_slider_wrapper' + data.id)) {
                    setW1(document.getElementById('main_slider_wrapper' + data.id).getBoundingClientRect().width)
                    setH1(JSON.parse(data.obj || "{}").trueHeight === 'true' ? document.getElementById('main_slider_wrapper' + data.id).getBoundingClientRect().width / 16 * 9 : document.getElementById('main_slider_wrapper' + data.id).getBoundingClientRect().height)
                } else {
                    setW1(document.getElementById('slider_wrapper' + data.id).getBoundingClientRect().width)
                    setH1(JSON.parse(data.obj || "{}").trueHeight === 'true' ? document.getElementById('slider_wrapper' + data.id).getBoundingClientRect().width / 16 * 9 : document.getElementById('slider_wrapper' + data.id).getBoundingClientRect().height)
                }
            } else {
            ...
            if(document.getElementById('sl'+data.id+'CoI0')){
                document.getElementById('sl'+data.id+'CoI0').style.transitionDuration = '12s'
            }
        }
    ...
    return(
    <div style={{}}>
        <div id={'sliderContainer' + data.id}
             onMouseDown={e=>{if((!JSON.parse(data.obj || "{}").scroll || JSON.parse(data.obj || "{}").scroll === 'Зажатием мыши') && (user.role < 4 || !document.location.pathname.includes(ADMIN_ROUTE))){setTouch(e.pageX)}}}
             onTouchStart={e=>{if((JSON.parse(data.obj || "{}").scroll || 'Зажатием мыши') === 'Зажатием мыши' && (user.role < 4 || !document.location.pathname.includes(ADMIN_ROUTE))){setMTouch(e.targetTouches[0].pageX)}}}
             onClick={()=>{if (JSON.parse(data.obj || '{}').isProduct) {inface.setCurrProduct(data);document.getElementById('theOnlyOneBody').style.overflow = 'hidden';} if((!user.role > 3 || !document.location.pathname.includes(ADMIN_ROUTE)) && JSON.parse(data.obj || "{}").fullScreen === 'true'){inface.setCurrImages(CLI);inface.setCurrImage(CS);}}} className='US' style={{transitionProperty:'transform,opacity',background:(JSON.parse(data.obj || '{}').sliderBackground || '#000000') + (JSON.parse(data.obj || "{}").shadow !== null?JSON.parse(data.obj || "{}").shadow * 255:0.5 * 255).toString(16).split('.')[0],color:JSON.parse(data.obj || "{}").sliderColor || 'inherit',display:'grid',gridGap:'0',gridTemplateColumns:'100%',zIndex:'1',position:'absolute',width:'100%',padding:!inface.mobile?'90px 10% 90px 10%':'50px 10px',height:'100%',placeItems:'center',fontSize:(JSON.parse(data.obj || "{}").fontSize || 1)+'em',cursor:(!user.role > 3 || !document.location.pathname.includes(ADMIN_ROUTE)) && JSON.parse(data.obj || "{}").fullScreen === 'true'?'pointer':'default'}}>
            {!JSON.parse(data.obj || '{}').isProduct && <div className={JSON.parse(data.obj || "{}").anim?'FDAStart':'FDAEnd'} onClick={()=>{if(user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)){inface.setCurrText(data)}}} style={{width:'100%',height:'auto'}} dangerouslySetInnerHTML={{__html:JSON.parse(data.obj || '{}').text || ''}}/>}
        </div>
        <div style={{position:'relative',width:'100%',height:h1 + 'px',overflow:'hidden',}}>
            <div id={'slider'+data.id} style={{transform:'translateX(' + -(w1 * (CS)) + 'px)',transitionProperty:'transform',transitionDuration:JSON.parse(data.obj || "{}").scroll === 'Наведением мыши'?'0s':'1.3s',left:(JSON.parse(data.obj || "{}").scroll || 'Зажатием мыши') === 'Зажатием мыши'?-(w1 * (CL > 0?CL > 1?CL > 2?2:2:1:0)) + 'px':'0',gridTemplateColumns:(JSON.parse(data.obj || "{}").scroll || 'Зажатием мыши') === 'Зажатием мыши'?'repeat('+(CL + 4)+', '+w1+'px)':'repeat('+CL+', '+w1+'px)',gridTemplateRows:'100%',position:'absolute',top:'0',minWidth:'100%',height:h1 + 'px',display:'grid',}}>
                ...
                <div style={{width:w1 + 'px',height:'100%',position:'relative',overflow:'hidden'}}>
                    <LazyImage style={{width:'100%',height:'100%',objectFit:JSON.parse(data.obj || '{}').isProduct ? 'contain' : 'cover',objectPosition:'center',}} src={process.env.REACT_APP_API_URL + d} alt=''/>
                </div>)}
                {CLI.map((d,key)=>{
                    ...
                    return(
                    <div style={{width:w1 + 'px',height:'100%',position:'relative',overflow:'hidden'}}>
                        <LazyImage id={'sl'+data.id+'CoI'+key}  style={{width:'100%',height:'100%',objectFit:JSON.parse(data.obj || '{}').isProduct ? 'contain' : 'cover',objectPosition:'center'}} src={process.env.REACT_APP_API_URL + d} alt=''/>
                    </div>
                    )
                })}
        ...
        {document.getElementById('main_slider_wrapper' + data.id) && <div className='slider-lateral-container'>
            <button onClick={()=>{if (CS > 0) {setCS(CS - 1)}}} style={{transform:inface.mobile && 'translateX(0)',opacity:inface.mobile && '1',fontSize:inface.mobile && '1.5em',padding:inface.mobile ? '0 8px 0 6px' : '0 10px 0 8px',left:'-20px',background:JSON.parse(inface.page.obj || '{}').color || '#022978'}} className='slider-lateral-buttons slider-left-btn'>&lt;</button>
            <button onClick={()=>{if (CS < CL - 1) {setCS(CS + 1)}}} style={{transform:inface.mobile && 'translateX(0)',opacity:inface.mobile && '1',fontSize:inface.mobile && '1.5em',padding:inface.mobile ? '0 6px 0 8px' : '0 8px 0 10px',right:'-20px',background:JSON.parse(inface.page.obj || '{}').color || '#022978'}} className='slider-lateral-buttons slider-right-btn'>&gt;</button>
        </div>}
        ...
        <div style={{textAlign:'center',width:'100%',position:'absolute',zIndex:'3',bottom:JSON.parse(data.obj || "{}").sliderButtons === 'Точки под слайдером'? JSON.parse(data.obj || '{}').isProduct ? '-20px' : '-20%':'7%'}}>
